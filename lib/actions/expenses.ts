...
function parseExpenseForm(formData: FormData): ParsedExpenseForm {
  ...
  if (!description) return { ok: false, error: "Opis je obavezan." };
  if (!Number.isFinite(amount) || amount <= 0) {
    return { ok: false, error: "Iznos mora biti veći od 0." };
  }
  if (!paidBy) return { ok: false, error: "Izaberi ko je platio." };
  if (participantIds.length === 0) {
    return { ok: false, error: "Izaberi bar jednog učesnika u podeli." };
  }
  ...
  if (splitMode === "custom") {
    ...
    if (shares.some((s) => !Number.isFinite(s.share_amount) || s.share_amount < 0)) {
      return { ok: false, error: "Svaki udeo mora biti broj veći ili jednak 0." };
    }
  } else {
